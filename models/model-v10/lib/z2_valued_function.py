...
def southPoleChart(coord, branchPoints, weights):
	...
	minPhi = jnp.min(branchPoints[0])
	maxPhi = jnp.max(branchPoints[0])
	result2 = stepDown(
		phi,
		maxPhi,
		jnp.pi)
	...
	if(config.SOUTHPOLE_IS_BRANCH_POINT):
		...
		liftedPhi = jnp.sqrt(jnp.pi - phi)
		liftedTheta = jnp.remainder(theta, 2.0 * jnp.pi) / 2.0
		...
		minTheta = jnp.min(branchPoints[1])
		maxTheta = jnp.max(branchPoints[1])

		result2 *= stepUp(theta, 0, minTheta)
		result2 *= stepDown(theta, maxTheta, 2.0 * jnp.pi)
		result2 = (1.0-result2) * stepUp(phi, 0, minPhi)

		return result1 * result2 * (jnp.pi - phi)
	else:
		# We use the xy coordinates as input of the neural network
		result1 = neuralNetwork.evaluate(coord[:-1], weights)[0]
		return result1 * (1.0 - result2)
...
def northPoleChart(coord, branchPoints, weights):
	...
	# the theta value of the branch points
	thetas = branchPoints[1]
	if(config.SOUTHPOLE_IS_BRANCH_POINT):
		thetas = jnp.append(thetas, 0.0)

	# We lift these coordinates to double cover
	liftedPhi = jnp.sqrt(phi)
	liftedTheta = jnp.remainder(theta -  thetas[0], 2.0 * jnp.pi) / 2.0
	liftedCoords = jnp.asarray([
		liftedPhi * jnp.cos(liftedTheta),
		liftedPhi * jnp.sin(liftedTheta)
	])
	...
	sign = 1.0
	for angle in thetas[1:]:  # This is a small loop, so no big performance panalty
		sign *= jnp.where(
			jnp.greater(
				theta,
...
def genericChart(coord, branchPointsIndex, branchPoints, weights):
	...
	if(config.SOUTHPOLE_IS_BRANCH_POINT):
		thetas = jnp.append(thetas, 0.0) # important to add to the end, as this keeps the branchPointsIndex indexed properly
