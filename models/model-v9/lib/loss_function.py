...
def minimumDistanceToGraph(coord, branchPoints):
	...
	for i in range(num):
		...
		# We want to remove the whole graph!
...
def minimumDistanceBetweenBranchPoints(branchPoints):
	result = jnp.inf;
...
def _unsafeLossFunction(coords, parameters, factor):
	...
	distance = minimumDistanceBetweenBranchPoints(parameters[4])
